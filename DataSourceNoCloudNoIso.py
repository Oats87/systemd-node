...
# that does not react to ISOs. This is specifically to allow ignoring ISO based cloud-init data that does not apply to the container (for example, lima)
...
class DataSourceNoCloudNoIso(sources.DataSource):
    ...
    def platform_type(self):
        ...
        if not self._platform_type:
            self._platform_type = "lxd" if util.is_lxd() else "nocloudnoiso"
...
def load_cmdline_data(fill, cmdline=None):
    pairs = [
        ("ds=nocloudnoiso", sources.DSMODE_LOCAL),
    ]
...
datasources = [
    (DataSourceNoCloudNoIso, (sources.DEP_FILESYSTEM,)),
]
